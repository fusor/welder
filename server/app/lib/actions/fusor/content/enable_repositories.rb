      ...
      class EnableRepositories < Actions::Base
        ...
        def plan(deployment)
          ...
          sequence do
            ...
            if content
              content[:rhev].each{ |details| enable_repo(details) } if content[:rhev]
              content[:cloudforms].each{ |details| enable_repo(details) } if content[:cloudforms]
              content[:openstack].each{ |details| enable_repo(details) } if content[:openstack]
            end
          end
        end
        ...
        def enable_repo(repo_details)
          ...
          product = ::Katello::Product.find_by_name(repo_details[:product_name])
          ...
          if product
            ...
            if repo_mapper(product, product_content.content, substitutions).find_repository
              Rails.logger.info("Repository already enabled for: Product: #{product.name}, Repository Set: #{product_content.content.name}")
            else
              plan_action(::Actions::Katello::RepositorySet::EnableRepository, product, product_content.content, substitutions)
            end
          else
